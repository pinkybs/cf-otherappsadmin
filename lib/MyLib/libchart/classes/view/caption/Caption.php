	...
	class Caption {
		...
		public function render() {
			...
			foreach ($this->labelList as $label) {
                ...
                if($i%3!=0 && $count>=20){
    			    $boxX1 = $captionArea->x1 + $lineWidth * ($i%3);
                    $boxX2 = $boxX1 + $this->labelBoxWidth;
                    $boxY1 = $captionArea->y1 + 2 + ($i-$i%3) * $lineHeight;
                    $boxY2 = $boxY1 + $this->labelBoxHeight;
			    } else {
                    $boxX1 = $captionArea->x1 ;
                    $boxX2 = $boxX1 + $this->labelBoxWidth;
                    $boxY1 = $captionArea->y1 + 2 + $i * $lineHeight;
                    $boxY2 = $boxY1 + $this->labelBoxHeight;
                }
                ...
                $text->printText($img, $boxX2 + 5, $boxY1 + $this->labelBoxHeight / 2, $this->plot->getTextColor(), $label, $text->fontCondensed, $text->VERTICAL_CENTER_ALIGN,12);
				$i++;
			}
		}
