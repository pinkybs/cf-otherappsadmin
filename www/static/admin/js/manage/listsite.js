...
$().ready(function() {
    ...
    $("#btnNew").click(function (){
        $('#frmBack').attr("action", UrlConfig.BaseUrl + '/manage/addsite');
	    $('#frmBack').submit();
	    return false;
    })
});
...
$.listsite = {
    ...
    changePage : function(pageIndex)
    {
        ...
        jQuery.ajax({
            type : "POST",
            url : UrlConfig.BaseUrl + '/ajax/manage/listsite',
            dataType: "json",
            data : {
                    pageIndex : pageIndex,
	...
	showInfo : function(array)
	{
	    ...
	    for (var i = 0 ; i < array.length ; i++) {        
	        ...
	        var linkEdit = '<a href="javascript:void(0);" onclick="$.listsite.editSite(' + array[i].app_id + ');return false;" >編集</a>';
	        var linkDel = '<a href="javascript:void(0);" onclick="$.listsite.delapp(' + array[i].app_id + ');return false;">削除</a>';
	        
	        html += '<tr class="' + cssClass + '">'
	              + '    <td>' + array[i].app_id + '</td>'
	              + '    <td><a href="javascript:void(0);" onclick="$.listsite.todetail(' + array[i].app_id + ');return false;" >' + array[i].app_name + '</a></td>'
	...
	todetail : function(id){
        $('#frmBack').attr("action", UrlConfig.BaseUrl + '/manage/appdetail/aid/' + id);
        $('#frmBack').submit();
        return false;
    },
	...
	editSite : function(id){
        $('#frmBack').attr("action", UrlConfig.BaseUrl + '/manage/editsite/aid/' + id);
        $('#frmBack').submit();
        return false;
    },
