...
$().ready(function() {
    var appErr = "0";
    $("#btnSubmit").click(function (){$.addowner.doSubmit();})
...
$.addowner = {
    ...
    {
        $.addowner.checkNameValue();
        $.addowner.checkIdValue();
        $.addowner.checkPWValue();
    ...
    doSubmit : function()
    {
        $.addowner.confirmValue();
        if( appErr == '1') {
            return;
        }
        var frmAdd = $('#frmAdd');
        frmAdd.attr("action", UrlConfig.BaseUrl + '/ajax/user/addowner');
        $("form#frmAdd").ajaxSubmit( function(data) { $.addowner.renderResultsSubmit(data); } );
...
/**
    ...
    { 
        if (response == '2') {  
			$('div#completeMsg').html('新規登録が完了しました。引き続き、他のアプリケーションオーナーの新規登録が行えます。');
